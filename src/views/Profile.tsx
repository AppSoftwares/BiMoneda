...
const Profile: React.FC = () => {
  ...
  const SubHeader = ({ title }: { title: string }) => (
    <header className="bg-white px-6 h-20 flex items-center gap-4 shadow-level-1 sticky top-0 z-50">
      <button onClick={() => setActiveSection('main')} className="p-2 text-primary active:scale-90 transition-transform">
  ...
  return (
    <div className="min-h-screen bg-background font-inter pb-32">
      {activeSection === 'main' && (
        <>
          <div className="bg-[#F9F7F2] pt-12 pb-10 flex flex-col items-center">
            <div className="relative">
              <div className="w-32 h-32 rounded-full border-4 border-[#C99A32] p-1 shadow-2xl bg-white overflow-hidden">
                <img src={avatarUrl || "/logo-1024.png"} className="w-full h-full object-cover rounded-full" />
              </div>
              <button
                onClick={() => fileInputRef.current?.click()}
                className="absolute bottom-1 right-1 w-10 h-10 bg-white rounded-full border border-outline-variant flex items-center justify-center shadow-lg active:scale-90 transition-all"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-primary" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H6a2 2 0 01-2-2V5zm3 4a1 1 0 000 2h.01a1 1 0 100-2H7zm3 0a1 1 0 000 2h3a1 1 0 100-2h-3zm-3 4a1 1 0 100 2h.01a1 1 0 100-2H7zm3 0a1 1 0 100 2h3a1 1 0 100-2h-3z" clipRule="evenodd" />
                </svg>
              </button>
              <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={(e) => uploadFile(e, 'profiles', 'avatar_url')} />
            </div>
            <h2 className="mt-6 text-3xl font-black text-[#0D2B5B] uppercase tracking-tighter italic">
          ...
          <main className="p-6 space-y-6 max-w-md mx-auto -mt-6">
            ...
            <div className="bg-white rounded-xl shadow-level-1 border border-outline-variant overflow-hidden">
              ...
              <MenuItem label={t('logout')} icon={<svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-6 0v-1m6-11V7a3 3 0 01-6 0v1" /></svg>} onClick={() => supabase.auth.signOut()} color="text-red-600" />
            </div>
          </main>
        </>
      )}
            ...
            <div className="bg-white rounded-lg border border-outline-variant shadow-level-1 p-6 space-y-6">
              <div className="space-y-4">
                ...
                <div className="space-y-2 border-b border-outline-variant/20 pb-4">
                  ...
                  <input value={editCompany.name} onChange={e => setEditCompany({...editCompany, name: e.target.value})} className="w-full text-sm font-bold text-primary bg-surface-container-low p-3 rounded-md outline-none border border-transparent focus:border-accent-sky" />
                </div>
                <div className="space-y-2 border-b border-outline-variant/20 pb-4">
                  <span className="text-[11px] font-bold text-on-surface-variant uppercase tracking-widest block">Dirección</span>
                  <textarea value={editCompany.address} onChange={e => setEditCompany({...editCompany, address: e.target.value})} className="w-full text-xs font-medium text-primary bg-surface-container-low p-3 rounded-md outline-none border border-transparent focus:border-accent-sky" rows={3} />
                </div>
                ...
                <div className="flex justify-between items-center pt-2">
                   ...
                   <button onClick={() => alert('Para cambiar el logo usa la foto de perfil en la pantalla principal.')} className="text-[10px] font-bold text-secondary uppercase tracking-widest px-3 py-1 bg-surface-container-low rounded border border-outline-variant/30">Edit</button>
                </div>
              </div>

              <button onClick={handleUpdateCompany} disabled={loading} className="w-full py-4 bg-[#055E5E] text-white font-bold rounded-md shadow-md active:scale-95 transition-all text-xs uppercase tracking-widest">
              ...
              <div className="bg-white rounded-lg border-2 border-dashed border-outline-variant/50 p-8 flex flex-col items-center justify-center min-h-[160px]">
                {profile?.signature_url ? (
                  <div className="relative group">
                    ...
                    <button onClick={() => signatureInputRef.current?.click()} className="absolute -top-4 -right-4 bg-white border border-outline-variant p-2 rounded-full shadow-md opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <div className="text-center space-y-4">
                    ...
                    <button onClick={() => signatureInputRef.current?.click()} className="px-6 py-2.5 bg-surface-container-low text-primary text-[10px] font-bold uppercase rounded border border-outline-variant/30 active:scale-95 transition-all">
                      {t('upload_signature')}
                    </button>
                  </div>
                )}
             ...
             <div className="bg-white rounded-lg border border-outline-variant shadow-level-1 overflow-hidden">
                <div className="p-5 space-y-6">
                   <div className="space-y-2">
                      ...
                      <button className="w-full py-3.5 bg-[#055E5E] text-white font-bold rounded-md text-xs uppercase tracking-widest mt-2 active:scale-95 transition-all">Guardar Cambios</button>
                   </div>
                   ...
                   <div className="space-y-4">
                      ...
                      <button className="w-full py-3.5 bg-[#055E5E] text-white font-bold rounded-md text-xs uppercase tracking-widest mt-2">Actualizar Contraseña</button>
                   </div>
                </div>
             </div>
        ...
        <>
          ...
          <main className="p-6 space-y-8 max-w-md mx-auto">
             <div className="space-y-2">
                ...
                <h3 className="text-[11px] font-bold text-[#C99A32] uppercase tracking-widest pt-4 ml-1">Seguridad de la Cuenta</h3>
             </div>
             <div className="bg-white rounded-xl shadow-level-1 border border-outline-variant overflow-hidden">
                ...
                <div className="p-5 flex items-center justify-between border-b border-outline-variant/20">
                   ...
                   <div className="w-12 h-6 bg-outline-variant rounded-full relative p-1"><div className="w-4 h-4 bg-white rounded-full shadow-sm"></div></div>
                </div>
             ...
             <h3 className="text-[11px] font-bold text-[#C99A32] uppercase tracking-widest ml-1">Legal</h3>
             <div className="bg-white rounded-xl shadow-level-1 border border-outline-variant overflow-hidden">
          ...
          <main className="p-6 space-y-8 max-w-md mx-auto">
             <div className="bg-white rounded-xl shadow-level-1 border border-outline-variant overflow-hidden">
                <div className="p-6 flex items-center justify-between border-b border-outline-variant/20">
                   ...
                   <button onClick={toggleTheme} className={`w-14 h-7 rounded-full p-1 transition-all ${theme === 'dark' ? 'bg-[#055E5E]' : 'bg-outline-variant'}`}>
                ...
                <div className="p-6 flex items-center justify-between">
                   ...
                   <div className="flex bg-surface-container-low p-1 rounded-md border border-outline-variant">
                      <button onClick={() => setLanguage('es')} className={`px-4 py-1.5 text-[10px] font-bold uppercase rounded transition-all ${language === 'es' ? 'bg-white text-primary shadow-sm' : 'text-on-surface-variant'}`}>ES</button>
                      <button onClick={() => setLanguage('en')} className={`px-4 py-1.5 text-[10px] font-bold uppercase rounded transition-all ${language === 'en' ? 'bg-white text-primary shadow-sm' : 'text-on-surface-variant'}`}>EN</button>
                   </div>
                </div>
             </div>
          </main>
          ...
          <main className="p-6 space-y-8 max-w-md mx-auto text-center">
             ...
             <div className="bg-white rounded-2xl shadow-level-1 border border-outline-variant p-6 flex items-center justify-between">
                <div className="flex items-center gap-4 text-left">
                   <div className="w-10 h-10 rounded-lg bg-surface-container-low flex items-center justify-center text-primary"><svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg></div>
                   <div>
                ...
                <div className="w-14 h-7 bg-[#055E5E] rounded-full p-1 flex items-center justify-end"><div className="w-5 h-5 bg-white rounded-full"></div></div>
             </div>

             <div className="bg-white rounded-2xl shadow-level-1 border border-outline-variant p-6 space-y-6">
                <h3 className="text-[11px] font-bold text-[#C99A32] uppercase tracking-widest text-left">Ajustes de Sonido</h3>
                <div className="flex items-center justify-between">
                   ...
                   <div className="w-12 h-6 bg-[#055E5E] rounded-full p-1 flex items-center justify-end"><div className="w-4 h-4 bg-white rounded-full"></div></div>
                </div>
                <div className="space-y-4 pt-2">
                   ...
                   <div className="w-full h-1 bg-surface-container-low rounded-full relative">
                      <div className="absolute inset-0 bg-accent-sky w-[70%] rounded-full"></div>
             ...
             <div className="bg-surface-container-low p-4 rounded-xl flex gap-3 text-left">
                <svg className="h-5 w-5 text-accent-sky shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
        ...
        <>
          ...
          <main className="p-6 space-y-8 max-w-md mx-auto text-center">
             ...
             <div className="bg-white rounded-3xl shadow-level-2 border border-outline-variant p-8 space-y-6">
                <div className="space-y-1">
                   <h3 className="text-xs font-bold text-[#055E5E] uppercase tracking-[0.2em]">Contacto Directo</h3>
                ...
                <button className="w-full py-4 bg-[#055E5E] text-white font-bold rounded-md shadow-md text-xs uppercase tracking-widest active:scale-95 transition-all">Llamar a Administración</button>
             </div>
          </main>
        </>
      )}
