...
const Invoice: React.FC = () => {
  ...
  const generatePDF = async () => {
    ...
    doc.text(clName, margin + 35, clientY, { align: 'justify', maxWidth: 65 });
    ...
    const obsText = `OBSERVACIONES: ${(invoice.observations || "Ninguna.").toString()}`;
    const obsLines = doc.splitTextToSize(obsText, pageWidth - (margin * 2));
    doc.text(obsLines, margin, footerStartY + 5, { align: 'justify', maxWidth: pageWidth - (margin * 2) });
    doc.setFontSize(6.5);
    ...
    doc.text(lines1, margin, footerStartY + 12, { align: 'justify', maxWidth: pageWidth - 30 });
    const lines2 = doc.splitTextToSize(disclaimer2, pageWidth - 30);
    doc.text(lines2, margin, footerStartY + 12 + (lines1.length * 3.5), { align: 'justify', maxWidth: pageWidth - 30 });
    doc.setFontSize(6);
    doc.setFont("helvetica", "normal");
